...
interface ReservationCheckoutPaymentSectionProps {
  hasBalance: boolean;
...
export function ReservationCheckoutPaymentSection({
  hasBalance,
  paymentMethods,
  balanceDue,
  allocatedTotal,
  ...
  togglePaymentMethod,
  updatePaymentAmount,
}: ReservationCheckoutPaymentSectionProps) {
  if (!hasBalance) return null;

  return (
    <>
      <div>
        <Label className="mb-2 block font-semibold">Payment Method(s)</Label>
        {paymentMethods.length === 0 ? (
          <div className="text-center py-6 text-muted-foreground">
            <p className="text-sm">No payment methods available.</p>
            <p className="text-xs mt-1">Add payment methods in Settings &rarr; Lists</p>
          </div>
        ) : (
          <div className="space-y-1.5">
            {paymentMethods.map((method) => {
              ...
              return (
                <div
                  key={method.id}
                  className={`px-2.5 py-2 rounded-md border transition-all ${isSelected ? "border-primary bg-primary/5" : "border-border"}`}
                >
                  <div className="flex items-center gap-2.5">
                    <div className="cursor-pointer" onClick={() => togglePaymentMethod(method.id)}>
        ...
        {remainingAmount > 0.01 && allocatedTotal > 0 && allocatedTotal < minimumRequired - 0.01 && (
        ...
        {minimumRequired < balanceDue && allocatedTotal < minimumRequired - 0.01 && allocatedTotal > 0 && (
          <div className="text-xs mt-1 px-2.5 text-red-600 dark:text-red-400">
            Min. down payment: {formatCurrency(minimumRequired)} ({rentDownPct}% of total minus already paid{creditApplied !== 0 ? " and credit applied" : ""}) — need {formatCurrency(minimumRequired - allocatedTotal)} more
          </div>
        )}
      </div>
    </>
