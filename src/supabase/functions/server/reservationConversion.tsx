...
export function registerReservationConversionRoutes(
  ...
  app.post("/make-server-918f1e54/reservations/convert-to-rental", async (c) => {
    try {
      ...
      // Order-level totals (for overpayment check and surplus calculation)
      ...
      const newPaymentsTotal = (payments || []).reduce((sum: number, p: any) => sum + (p.amount || 0), 0);
      ...
      // ── Order-level minimum validation with per-item-type awareness ──
      // The frontend uses actual order payments (not proportional allocation) and
      // computes balance/minimum at the order level. The backend must match.
      
      // Fetch all items in this order to calculate type-aware minimum
      ...
      // Calculate order-level minimum: each item type has its own down payment %
      // Sale items (is_sale=true) → 100%, rentals (checked_out) → rentDownPct%, 
      // reservations (being converted to rental) → rentDownPct%
      let orderMinimumTotal = Math.round(grandTotal * rentDownPct / 100); // fallback
      if (allOrderItems && Array.isArray(allOrderItems)) {
        ...
        for (const oi of allOrderItems) {
          ...
          // Sale items always require 100%, all others use rental down payment %
          // (reserved items are being converted TO rental, so they also use rentDownPct)
          const isSale = (oi as any).is_sale === true;
          const downPct = isSale ? 100 : rentDownPct;
          orderMinimumTotal += Math.round(oiTotal * downPct / 100);
        }
      }

      // additionalMinimum: how much more needs to be paid to meet the order minimum
      const additionalMinimum = Math.max(0, orderMinimumTotal - existingPayments - creditAppliedAmount);

      console.log('💰 [CONVERT] Order-level minimum validation:', {
      ...
      // Validate: new payments + credit applied must not exceed order-level balance due
      if (newPaymentsTotal + creditAppliedAmount > balanceDue + 0.01) {
        return c.json({ error: "Payment + credit exceeds balance due. Overpayment is not allowed." }, 400);
      }

      // Validate: new payments must meet order-level minimum requirement
      // NOTE: creditApplied is already factored into additionalMinimum, so do NOT add it here
      // (adding it would double-count: once reducing the minimum, once reducing the payment)
      if (newPaymentsTotal < additionalMinimum - 0.01) {
        return c.json({ error: "Payment + credit is below minimum required. Rental requires " + rentDownPct + "% upfront." }, 400);
      }
